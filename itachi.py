...
def bcall_gen(client, message, bin_input):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(gen_cmd(client, message, bin_input))
    except Exception as e:
        logging.error(f"Error in bcall_gen: {e}")
    finally:
        loop.close()

async def gen_cmd(client, message, bin_input):
    ...
    try:
        getbin = await get_bin_details(bin_input[:6])
    ...
    if amount > 10000:
        ...
        return
    try:
        all_cards = await luhn_card_genarator(bin_input, None, None, None, amount)
    except Exception as e:
...
def register_command(message):
    ...
    if register_user(user_id):
        ...
        bot.reply_to(message, f"🎉 Registration successful! You have been granted a <b>1-hour</b> plan. Use /start to unleash your inner Goku!", parse_mode="HTML")
    else:
...
def callback_help_menu(call):
    help_text = """
<b>Goku Bot - Help Menu</b>

<b>User Commands</b>
/register - 𝐑𝐞𝐠𝐢𝐬𝐭𝐞𝐫 to join the Goku force!
...
/about - About Goku Bot - know your tools!
...
/ping - Check bot latency - is it faster than Goku?
...
For any questions, contact @GOKUOFFICIALREAL_BOT - your Senzu Bean for bot issues!
"""
...
def delete_user(message):
    ...
    try:
        ...
        if user_id_to_delete in allowed_users and user_id_to_delete not in admin_ids:
            ...
            bot.reply_to(message, "You cannot delete admin users, even Goku can't do that.")
            time.sleep(0.1)
        else:
...
def redeem_code(message):
    ...
    try:
        ...
        bot.reply_to(message, "Please provide a valid redeem code, Saiyan. Example: /redeem GOKU-XXXX-XXXX-XXXX")
        time.sleep(0.1)
        return
    ...
    if redeem_code_input in redeem_codes_data:
        ...
        if current_plan and current_plan['plan_type'] == 'lifetime':
            bot.reply_to(message, "You already have a lifetime subscription, like Goku's power! This code cannot be applied.")
            time.sleep(0.1)
            return
        if plan_duration:
            ...
            bot.reply_to(message, "Error processing redeem code duration. Contact @GOKUOFFICIALREAL_BOT for Senzu Beans (help).")
            time.sleep(0.1)
            return
        if str(user_id) not in allowed_users:
            allowed_users.append(str(user_id))
...
def gen_command_handler(message):
    ...
    try:
        ...
        ccs_in_message = extract_ccs_from_line(message.text)
        if ccs_in_message:
            bin_input = ccs_in_message[0].split('|')[0][:6]
        else:
            bot.reply_to(message, "❌ Invalid Format! Use:\n`/gen bin amount` or reply to a message containing bin with `/gen amount`", parse_mode="Markdown")
            return

    bcall_gen(None, message, bin_input)
...
def process_check(message, cc_input):
    ...
    if not session:
        bot.reply_to(message, "❌ Failed to create or load session. Please try again, Goku needs a stable connection too!")
        time.sleep(0.1)
        return
...
def check_single_card_command(message):
    ...
    ccs_in_message = extract_ccs_from_line(message.text)
    if ccs_in_message:
        cc_input = ccs_in_message[0]
        process_check(message, cc_input)
    else:
        bot.reply_to(message, "❌ 𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐂𝐂 𝐟𝐨𝐫𝐦𝐚𝐭 𝐨𝐫 𝐧𝐨 𝐂𝐂 𝐝𝐞𝐭𝐞𝐜𝐭𝐞𝐝. 𝐔𝐬𝐞: `/chk cc|mm|yy|cvv` or `.chk cc|mm|yy|cvv` or reply to a message containing cc with `/chk` or `.chk`")
        time.sleep(0.1)
...
def main(message):
    ...
    if message.reply_to_message and message.reply_to_message.document:
        ...
        if command in ['.chk', '/chk', '.mchk', '/mchk']:
            dd = 0
            live = 0
            incorrect = 0
            ...
            time.sleep(0.1)
            try:
                ee = bot.download_file(bot.get_file(message.reply_to_message.document.file_id).file_path)
                with open("combo.txt", "wb") as w:
                    w.write(ee)
            except Exception as e:
                logging.error(f"Error downloading or saving document: {e}")
                bot.edit_message_text(chat_id=message.chat.id, message_id=ko_msg_id, text='❌ 𝐅𝐚𝐢𝐥𝐞𝐝 𝐭𝐨 𝐝𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐟𝐢𝐥𝐞. Even Goku has trouble sometimes!')
                return

            try:
                session = manage_session_file()
                if not session:
                    bot.reply_to(message, "❌ Failed to create or load session. Please try again, even Goku needs to rest and recharge!")
                    time.sleep(0.1)
                    return
...
<a href='https://t.me/+bkxW-_IANZQ3YjY1'>-</a> 𝐁𝐨𝐭 𝐀𝐛𝐨𝐮𝐭: <a href='https://t.me/+bkxW-_IANZQ3YjY1'>Goku </a>'''
                ...
                bot.edit_message_text(chat_id=message.chat.id, message_id=ko_msg_id, text='𝗕𝗘𝗘𝗡 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗 ✅\n𝗕𝗢𝗧 𝗕𝗬 ➜ @GOKUOFFICIALREAL_BOT - Checks complete, Saiyan!')
                time.sleep(2)
        else:
            bot.reply_to(message, "❌ Use /chk, .chk, /mchk, or .mchk when replying to a document for mass checking.")
    else:
        bot.reply_to(message, "❌ Please reply to a document with /chk, .chk, /mchk, or .mchk for mass checking.")
...
/id - Get Your Telegram ID, or someone else's!
/ping - Check bot latency - is it faster than Goku?
/gen bin amount - Generate CCs (Admin only Super Saiyan Mode)
...
This bot is your training partner for card checking and generation, powered by Goku levels of strength!

<b>👨‍💻 Developer:</b> @GOKUOFFICIALREAL_BOT - Your Master Roshi for bot support

<b>📢 Channel:</b> @GOKUOFFICIALREAL_BOT - Get your daily dose of bot power-ups!

Just like Goku always strives to be stronger, we're constantly improving the bot. For support or inquiries, contact the developer, and let's power up together!
"""
...
<b>👥 Total Bot Users:</b> {bot_users_count} - The Goku Bot force grows!
"""
...
def list_bot_users_command(message):
    ...
    if not bot_user_ids:
        ...
        time.sleep(0.1)
    users_list_text = "<b>👥 All Bot Users - The Goku Force 👥</b>\n\n"
...
def broadcast_command(message):
    ...
    try:
        ...
        bot.reply_to(message, "Please provide a message to broadcast. Example: /broadcast Hello everyone! - Send out the Kamehameha message!")
        time.sleep(0.1)
    bot_user_ids = load_bot_users()
    broadcast_count = 0
    error_count = 0
    for user_to_broadcast in bot_user_ids:
        try:
            bot.send_message(user_to_broadcast, f"<b>📢 Broadcast Message from Admin - Goku Level Announcement! 📢</b>\n\n{broadcast_message}", parse_mode="HTML")
            broadcast_count += 1
            time.sleep(0.1)
        except Exception as e:
...
def ping_command(message):
    ...
    try:
        ...
        bot.reply_to(message, "Error during ping. Please check bot logs. Even Goku stumbles sometimes.")
...
for command, handler in command_handlers:
    ...
    def wrapped_handler(message):
        ...
        try:
            ...
            bot.reply_to(message, "❌ An error occurred while processing your command. Please try again later. Even Goku needs a Senzu Bean sometimes!")
